...
class AirsimEnv(gym.Env):
    ...
    def get_trgt_objects(self):
        ...
        for i, kp_list in enumerate(kp_per_cluster):
            kp_pts = np.array([kp.pt for kp in kp_list])
            ax.plot(kp_pts[:,0], kp_pts[:,1], 'o', color=colors[i], markersize=2)
        ...
        for i in range(n_clusters):
            ...
            for j, good in enumerate(good_list):
                print("Number of matches for cluster {}, ref img {}: {}".format(i, j, len(good)))
